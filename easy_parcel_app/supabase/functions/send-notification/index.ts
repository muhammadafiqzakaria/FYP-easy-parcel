...
// You must set these in your Supabase project's secrets
...
serve(async (req) => {
  try {
    ...
    // This client has full admin rights
    const supabase = createClient(
      Deno.env.get("SUPABASE_URL")!,
      Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!
    );

    // --- 4. Get the new parcel from the trigger ---
    const payload = await req.json();
    const newParcel = payload.record;

    console.log("New parcel received:", newParcel.id);

    // --- 5. Find the student's FCM token ---
    const { data: profile, error: profileError } = await supabase
      .from("profiles")
      .select("fcm_token")
      .eq("id", newParcel.studentId) // Match by student ID
      .single();
    ...
    // --- 6. Define the Notification Message ---
    const message = {
      notification: {
    ...
    // --- 7. Send the Notification via FCM ---
